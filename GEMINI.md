...
[Riot/Valorant/Bundles] `fetchStorefront` retourne aussi `bundles: RawBundle[]` extraits de `FeaturedBundle` (filtrage par `SKIN_TYPE_ID = e7c63390...`). `fetchBundleMeta(uuid)` récupère nom + image depuis valorant-api.com (`displayIcon2` en priorité). Dans `ValorantDailyShop.vue` : `expiresAt` calculé au chargement, `bundleNow` ref mis à jour chaque seconde pour un timer live "Xj Xh Xmin". Layout carte : bannière pleine largeur 220px + info row (nom/prix/discount à gauche, timer à droite) + grille skins (72px). Script Node `doc/valorant-skin-daily/refresh-auth.js` affiche aussi les packs via `getBundles()` avec `buildSkinMap()` partagé.
